# 继续完善美国传统词典的条目，给它添加韦氏词典和美国传统词典第五版的发音信息
# 合并词典文件，给美国传统词典添加韦氏词典的发音信息
import datetime
...
filepath = Path(r"C:/123pan/Downloads/美国传统英语词典英汉双解第3版/美国传统英语词典英汉双解第3版/美国传统英语词典英汉双解第3版.txt")
...
def process(v: str) -> str:
    # 数据清洗和格式调整
    v = v.replace(".See Synonyms at ", '. <span class="see-synonyms">See Synonyms at </span>')
    v = v.replace("参见 <b>", ' <span class="see-synonyms_cn">参见 </span><b>')
    v = re.sub(r"<b>([^<]*?)Abbr\.", r'<b>\1</b> <b>Abbr.', v)
    ...
    if v.find(links) == -1: # 避免重复添加
        ...
        # 放在开头的link标签之后 <link rel="stylesheet" href="ahd3af.css" />
        link = '<link rel="stylesheet" href="ahd3af.css" />'
        link_len = len(link)
        v = v[:link_len] + links + v[link_len:]
...
def main() -> int:
    ...
    # 构建第二个文件的 key -> 发音
    pronunciation_map2 = {}
    pattern1 = re.compile(r'((?:\([^)]*?|• \s*<b>[^<]*?</b>\s*|<i>or [^<]*?</i>\s*<b>[^<]*?</b>\s*)?<a href="sound:[^\\]*?\\\\[^\\]+?\\\\.*?)(?:<br>|<div |$)', flags=re.DOTALL) # 发音部分
    pattern2 = re.compile(r'<font style="font-weight:bold;">[^<]*?</font>', flags=re.DOTALL) # 带音节划分的词头
    ...
    # 构建第三个文件的 key -> 发音
    pronunciation_map3 = {}
    pattern3 = re.compile(r'<div class="rtseg">(.*?<a \s*class="sound" \s*href=.*?)</div>') # 发音部分
    ...
    # 合并发音到第一个文件的条目中
    old_wmcd11 = re.compile(r'<span class="wmcd11-pron">.*?</span><br><!-- wmcd11-pron_end -->', flags=re.DOTALL)
    old_ahd5 = re.compile(r'<span class="ahd5-pron">.*?</span><br><!-- ahd5-pron_end -->', flags=re.DOTALL)
    for i, (k, v) in enumerate(entries):
        ...
        text_to_add = ""
        if pronunciation2:
            if v.find(r'<span class="wmcd11-pron">') != -1: # 避免重复添加
                v = v.replace(old_wmcd11, '') # 先移除旧的
            text_to_add += f'<span class="wmcd11-pron">WMCD11: {pronunciation2}</span><br><!-- wmcd11-pron_end -->'
        if pronunciation3:
            if v.find(r'<span class="ahd5-pron">') != -1: # 避免重复添加
                v = v.replace(old_ahd5, '') # 先移除旧的
            text_to_add += f'<span class="ahd5-pron">AHD5: {pronunciation3}</span><br><!-- ahd5-pron_end -->'
